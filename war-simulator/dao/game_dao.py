from sqlalchemy import create_engine, Integer, Column, String, ForeignKey, select
...
class GameDao:
    ...
    def map_to_game_entity(game: Game) -> GameEntity:
        ...
        for player in game.get_players():
            ...
            battlefield_entity = GameDao.map_to_battlefield_entity(
                player.get_battlefield())
            vessel_entities = \
                VesselDao.map_to_vessel_entities(player.get_battlefield().id,
    ...
    def map_to_game(game_entity: GameEntity) -> Game:
        ...
        for player_entity in game_entity.player:
            player = Player(id = player_entity.id, name = player_entity.name)
...
class PlayerDao:
    def map_to_player_entity(player: Player) -> PlayerEntity:
        ...
        player_entity.battle_field = GameDao.map_to_battlefield_entity(player.get_battlefield())
    ...
    def create_player(self, player: Player) -> int:
        player_entity = self.map_to_player()
...
class VesselDao:
    ...
    def map_to_vessel_entities(battlefield_id: int, vessels: list[Vessel]) \
        ...
        for vessel in vessels:
            vessel_entity = VesselDao.map_to_vessel_entity(battlefield_id, vessel)
